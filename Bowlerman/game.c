...
PRIVATE void LoadPlayerTextures(Game newGame, Player *player, int ID, char sourceText[]);
...
struct game_type
{
    ...
    SDL_Texture *player_texture[4][13]; // 4  players, måste stå 4 annars blir de segmentation fault.
...
PRIVATE void LoadPlayerTextures(Game newGame, Player *player, int ID, char sourceText[40])
{
    char txt[40];
    strcpy(txt, sourceText);
    for (int i = 0; i < 12; i++)
    {
        newGame->player_texture[ID][i] = (SDL_Texture *)loadMedia(newGame, txt);
        printf("%s\n", txt);
        txt[26]++;
        if (txt[26] == '9')
        {
...
PRIVATE void initGame(Game newGame, Player *player)
{
    ...
    SDL_FreeSurface(newGame->window_surface);
    LoadPlayerTextures(newGame, player, 0, "bowlermantestskins/bowman00.png");
    // newGame->player_texture[0][0] = (SDL_Texture *) loadMedia(newGame, "bowlermantestskins/bowman00.png");
    LoadPlayerTextures(newGame, player, 1, "bowlermantestskins/bowman00.png");
    LoadPlayerTextures(newGame, player, 2, "bowlermantestskins/bowman00.png");
    LoadPlayerTextures(newGame, player, 3, "bowlermantestskins/bowman00.png");
    
    /*newGame->player_texture[1][0] = (SDL_Texture *)loadMedia(newGame, "pin2.png");
    SDL_FreeSurface(newGame->window_surface);
    newGame->player_texture[2][0] = (SDL_Texture *)loadMedia(newGame, "pin2.png");
    SDL_FreeSurface(newGame->window_surface);
    newGame->player_texture[3][0] = (SDL_Texture *)loadMedia(newGame, "pin2.png");
    SDL_FreeSurface(newGame->window_surface);*/
    ...
    for (int i = 0; i < playerAmmount; i++)
    {
        for(int j = 0; j < 13; j++)
        {
            SDL_QueryTexture(newGame->player_texture[i][j], NULL, NULL, &playerRect[i].w, &playerRect[i].h);
        }
        playerRect[i].w /= 4; // scales down width by 4
        playerRect[i].h /= 4; // scales down height by 4
    }
}
...
PRIVATE void updateMedia(Game newGame, SDL_Rect playerRect[], Player *player, int renderOrder[])
{
    ...
    for (int i = 0; i < playerAmmount; i++)
    {
        SDL_RenderCopy(newGame->renderer, newGame->player_texture[renderOrder[i]][2], NULL, &playerRect[renderOrder[i]]);
    }
